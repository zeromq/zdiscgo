...
##
[ -z "$CI_TIME" ] || echo "`date`: Build zdiscgo from local source"

(android_build_verify_so "libzdiscgo.so" "libczmq.so" &> /dev/null) || {
    rm -rf "${cache}/zdiscgo"
    (cp -r ../.. "${cache}/zdiscgo" && cd "${cache}/zdiscgo" \
        && make clean && rm -f configure config.status)

    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'

    (cd "${cache}/zdiscgo" && $CI_TIME ./autogen.sh 2> /dev/null \
...
android_build_verify_so "libzdiscgo.so" "libczmq.so"
